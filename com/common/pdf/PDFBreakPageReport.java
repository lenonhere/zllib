...
//须设置每页行数,程序人为分页,显示效果比较好

public class PDFBreakPageReport {
	...
	private int pageRows = 25; // 每页显示行数
	...
	public PDFBreakPageReport(String title, List captionResult, List dataResult) {
		this.title = title;
	...
	private void writeData(OutputStream bos) throws Exception {
		try {
			...
			for (int n = 0; n < hPages; n++) {
				...
				// 计算纵向分页数
				int totalRows = dataResult.size();
				int vPages = (totalRows - headRowCount)
						/ (pageRows - headRowCount);
				if (totalRows - headRowCount > vPages
						* (pageRows - headRowCount))
					vPages++;

				for (int m = 0; m < hPages; m++) {
					...
					int thisPageRows = m < (vPages - 1) ? pageRows
							: (totalRows - (vPages - 1)
									* (pageRows - headRowCount));

					List pageDataList = new ArrayList();
					for (int i = 0; i < thisPageRows - headRowCount; i++) {
						// System.out.println("get:"+(n*(pageCols-fixCols)+fixCols+i));
						pageDataList.add(dataResult.get(m
								* (pageRows - headRowCount) + i));
					}

					int control[][] = new int[thisPageRows][pageCaptionList
							.size()];
					for (int j = 0; j < pageDataList.size(); j++) {
						Object obj = pageDataList.get(j);

						for (int i = 0; i < pageCaptionList.size(); i++) {
							if (control[j][i] != 1) {
								ColStruct rows = (ColStruct) pageCaptionList
										.get(i);
								...
								if (i < colMergeCount) {
									for (int k = j + 1; k < pageDataList.size(); k++) {
										try {
											obj = pageDataList.get(k);
