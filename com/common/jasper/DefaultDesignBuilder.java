...
public class DefaultDesignBuilder extends DesignBuilder {
    ...
    protected void generatePageHeader() {
        ...
        if( parameters.get("position_left") != null) {
            ...
            int width = Integer.parseInt((String)parameters.get("position_left"));
        ...
        if( parameters.get("position_center") != null) {
            ...
            int width = Integer.parseInt((String)parameters.get("position_center"));
            int columnWidth = paper.getColumnWidth();
            setStaticTextAll(staticTexts, 1, (columnWidth - width) / 2 , y, width, 20, text);
            DesignMaker.setTextAlignment(staticTexts[1], ALIGN_CENTER, ALIGN_TOP);
        }

        //position 3
        if( parameters.get("position_right") != null) {
            String text = (String)parameters.get("position_right_text");
            int width = Integer.parseInt((String)parameters.get("position_right"));
            int columnWidth = paper.getColumnWidth();
            setStaticTextAll(staticTexts, 2, columnWidth - width, y, width, 20, text);
            DesignMaker.setTextAlignment(staticTexts[2], ALIGN_RIGHT, ALIGN_TOP);
        }
