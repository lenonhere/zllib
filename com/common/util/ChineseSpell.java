...
public class ChineseSpell {
	 ...
	 * @return
	 */
	public static boolean isChineseLetter(int ascii) {
	 ...
	 * @return
	 */
	public static String getFirstLetter(char cn) {
		int ascii = getAscii(cn);
		if (isChineseLetter(ascii)) {
			String spell = getSpellByAscii(ascii);
	 ...
	 * @return
	 */
	public static String getFirstLetters(String cnStr) {
		if (null == cnStr || "".equals(cnStr.trim())) {
			return cnStr;
		}
		char[] chars = cnStr.toCharArray();
		...
		for (int i = 0, Len = chars.length; i < Len; i++) {
			String letter = getFirstLetter(chars[i]);
			if (letter != null)
				sb.append(letter);
		}
		return sb.toString();
